...
startDate = today.replace(day=1, month=startMonth, year=year)
endDate = today.replace(year=startDate.year+1, month=1,
                        day=1) if startDate.month == 12 else today.replace(month=startDate.month+1, day=1)
...
df = pd.DataFrame({"Dato": pd.to_datetime(
    dates, utc=True), "kWh": kWhMeasurements})
if (df.empty):
